  ...
  Future<TrainingPointResponse> fetchTrainingPoints() async {
    ...
    try {
      ...
      if (response.data is! Map<String, dynamic>) {
        print('fetchTrainingPoints: ERROR - Data is not a Map: \${response.data.runtimeType}');
        throw Exception('Invalid response format: expected Map, got \${response.data.runtimeType}');
      }
      
      final data = TrainingPointResponse.fromJson(response.data as Map<String, dynamic>);
      print('fetchTrainingPoints: PARSED SUCCESSFULLY');
      return data;
    } catch (e, st) {
      print('fetchTrainingPoints: EXCEPTION ' + e.toString());
      rethrow;
    }
  }
}
