...
void main() {
  test(
    'AiPortalContextBuilder strips credentials and limits schedules',
    () async {
      final container = ProviderContainer(
        overrides: [
          ...
          scheduleFutureProvider.overrideWith(
            (ref) async => const ScheduleResponse(
              hocKy: 2,
              namHoc: 2026,
              tiets: [
                ScheduleItem(
          ...
          tuitionListProvider.overrideWith(
            (ref) async => [
              TuitionRecord(
      ...
      final snapshot = await builder.preload(container, {
      ...
      expect(
        ...
        isNot(contains('Nguyen Van A')),
      ); // Bị loại bỏ
      expect(
        snapshot.profileSummary,
        isNot(contains('23520804')),
      ); // Bị loại bỏ
